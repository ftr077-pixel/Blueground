...
> - **A9 (2026-08-02, requested by Denis):** M2's call history arrives early: calls, their bilingual transcripts, per-stage timings and a post-call summary are persisted and browsable per operator. §11 says Postgres + SQLAlchemy; this uses **SQLite** through the standard library instead — one box, eight operators, and one file that `cp` backs up, against a database server, a driver dependency and an ORM for data that is written once at the end of a call and read by a handful of people. The swap point is M3's horizontal scaling, where session affinity and concurrent writers make a real server necessary; the schema is deliberately plain SQL so that migration is a data copy. Summaries use `MT_MODEL_QUALITY`, already configured under ADR-003's quality tier and until now unused. Access is per-operator, with an `ADMIN_EMAILS` list that sees everything; both fail closed. Transcripts of real customer conversations are now on disk, so §12's retention question stops being theoretical — `CALL_RETENTION_DAYS` prunes them and defaults to 90.
> - **A8 (2026-08-02, requested by Denis):** §9's operator auth (M2) lands early, as Google sign-in. The shape is forced by ADR-001: Twilio cannot answer a login prompt, so `/twilio/voice` and `/ws/twilio` stay open and the guard covers `/operator`, `/ws/operator`, `/call` and `/debug/*`. Identity is an allowlist of Google accounts, fail-closed. §11 dependencies: +`itsdangerous` (signs the session cookie for Starlette's `SessionMiddleware`); the OAuth exchange itself uses `httpx`, already present. Verifying Twilio's request signature on the routes that stay open remains M2 work.
